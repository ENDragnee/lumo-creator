...
import { TextSettings } from "./settings/TextSettings"; // <-- Updated path
import 'katex/dist/katex.min.css'; // <-- Import KaTeX CSS for rendering

// --- TextProps Interface ---
export interface TextProps {
    text: string; // Changed from 'content' to 'text' for clarity
    width?: string | number;
    ...
    tagName?: 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6' | 'p' | 'div'; // Added tagName
}
...
export const TextComponent: CraftableComponent = ({
  ...
  width = "100%",
  padding = "8px",
  tagName = 'p', // Default to paragraph
}) => {
  const {
  ...
  // Sync state when the 'text' prop changes from outside
  useEffect(() => {
    if (!isEditing) {
  ...
  const handleFinishEditing = useCallback(() => {
    ...
    if (localContent.trim() !== text.trim()) {
      setProp((props: TextProps) => { props.text = localContent; }, 500);
    }
  ...
  // The component that will be rendered by ReactMarkdown
  ...
  return (
    <div
        ...
        style={rootStyle}
        className={`relative ${editorEnabled ? 'cursor-grab' : ''} text-component-wrapper`}
...
// The rich, specific props are now defined in the TextToolPanel.
TextComponent.craft = {
  displayName: "Text",
  props: {
